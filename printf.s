...
;\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
;       Wrapper for printf() to handle stdcall
;       convention
;       rdi - format string
;       rsi rdx rcx r8 r9            \
;       [rsp + 2*8] [rsp + 3*8] ...  - arguments
;exit:  None
...
_printf_stdcall:
        ...
        add rsp, 6*8    ;restore stack size
...
.x:
        call _itoa_h			;rax=strlen
        jmp .endnum

.b:
        call _itoa_b			;rax=strlen
        jmp .endnum

.o:
        call _itoa_o			;rax=strlen
        jmp .endnum
...
.symb:
        inc rdx				;move along format string
...
section .bss
.CONVBUF:	db 64 dup (?)		;buffer for num2ascii convertion
section .text
