...
.nfind:	
        mov rax, 0x01	;write(rdi, rsi, rdx) ... r10, r8, r9
        mov rdi, 1	;stdout
        mov rsi, Var
...
.loop:	
        cmp byte [rdi], 0	;check termination
        je .end
...
.end:	
        cmpsb			;1=end,2=end->zf=1; 1=end,2!=end->zf=0,cf=0;
        ret
...
.loop: 	
        ...
        shr rax, cl		;3*dig_pos shif
        and rax, 0111b	     	;get last 4 bits (oct digit)

        add al, '0'
        stosb

        test rcx, rcx           ;rcx=0 -> zf=1
        jnz .loop
...
;	[rdi]	- dst string address
;	rbx	- radix
;	WARNING:  64 bytes in dest string required
;exit:	rax	- dst strlen
;	[rdi]	- byte after converted number
...
_itoa:
        ...
        lea rsi, [rdi + 64]	;mov rsi to string_end+1
...
.cont:	
        xor rcx, rcx		;counts converted number length	

.conv: 	
        xor rdx, rdx		;clean rdx for div
        div rbx			;(rdx,rax)/rbx
